...
Inputs: `projects/<slug>/{script.md,voiceover.md}`, `work/transcript.json` (run step 1 if missing), `config/profile.json` (esp. `rules[]`), `memory/*.md`.
...
4. **Inserts**: every `(后期提示：…)` in the script gets a card or a `todo`. Maps for places, `quote` for references, `checklist` for lists of 3–5. Prefer a card over a pop when there are > 3 words.
