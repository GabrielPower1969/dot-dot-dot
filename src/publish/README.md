...
`node src/publish/queue.mjs run` (cron/launchd every 15 min): for rows due and `confirmed`, call the adapter's `submit`; otherwise `prepare` and leave a screenshot in `publish/screens/`.
